...
const DismissCircleRegular = () => {
    return <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 48 48" className="shrink-0 w-8 h-8">
        <path
...
const BalloonTypeMap = {
    [BalloonTypes.Info]: "bg-[#0078D4]",
    [BalloonTypes.Warn]: "bg-amber-400 text-black",
    [BalloonTypes.Alert]: "bg-red-600",
...
export function AlertProvider({ children }: { children: ReactNode }) {
    ...
    return (
        <AlertContext.Provider value={{ createAlertBalloon: createAlertBalloon }}>
            {children}
            <div id="alert-floater" className="fixed top-5 left-1/2 transform-[translate(-50%)]">
                {alerts.map(alert => (
                    <div className={"mb-2.5 p-2.5 flex flex-row justify-between items-center rounded-lg max-w-md " +
                        BalloonTypeMap[alert.type] } style={{ boxShadow: "black 0 0 10px" }} key={alert.id}>
                        <div className="flex flex-row flex-nowrap align-center">
                            <span className="text-[2rem] flex flex-col flex-nowrap justify-center mr-2.5">
                                { IconMap[alert.type] }
                            </span>
                            <div>
                                <h3 className="leading-none font-bold text-xl">{ alert.title }</h3>
                                <p className="leading-none">{ alert.message }</p>
                            </div>
                        </div>
                        <DismissRegular className="rounded-lg ml-5 p-2 h-10 w-auto hover:bg-[#fff4] active:bg-[#fff5]"
